...
CONFIG_PATH="${CONFIG_PATH:-configs/railsem19/segformer_b0_rs19_512x512_150ep_rtx4090.py}"
WORK_DIR="${WORK_DIR:-runs/rs19/segformer_b0_512x512_150ep_rtx4090}"
DRY_RUN="${DRY_RUN:-0}"

# Optional checkpoints
# Strict resume (恢复优化器/调度器状态):
#   RESUME_CKPT="runs/rs19/segformer_b0_512x512_50ep_rtx4090/iter_xxx.pth" ./scripts/server_train_50ep_rtx4090.sh
# Weights-only load (推荐):
#   LOAD_CKPT="runs/rs19/segformer_b0_512x512_80000it_server/best_mIoU_iter_79000.pth" ./scripts/server_train_50ep_rtx4090.sh
RESUME_CKPT="${RESUME_CKPT:-}"
LOAD_CKPT="${LOAD_CKPT:-}"

if [[ ! -d "$PROJECT_ROOT" ]]; then
  echo "[ERR] PROJECT_ROOT not found: $PROJECT_ROOT"
  echo "[TIP] set PROJECT_ROOT explicitly, e.g. PROJECT_ROOT=$PWD ./scripts/server_train_50ep_rtx4090.sh"
  exit 1
fi
...
# Important: enable custom dataset/hook imports under src/
export PYTHONPATH="$PROJECT_ROOT/src:${PYTHONPATH:-}"

mkdir -p "$WORK_DIR"

LOG_FILE="$WORK_DIR/server_train_50ep_rtx4090_$(date +%Y%m%d_%H%M%S).log"
...
echo "[OK] 50-epoch training started in background"
echo "[OK] log: $LOG_FILE"
echo "[TIP] monitor: tail -f $LOG_FILE"
echo "[TIP] per-epoch record: val/checkpoint/logger all use interval=1700 iters"
