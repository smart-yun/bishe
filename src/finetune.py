...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Finetune a structurally pruned SegFormer on RailSem19."
    )
...
def main():
    ...
    # Keep a CPU template for reconstructing the best full model after training.
    pruned_model_template = copy.deepcopy(model).cpu()

    # Replace the freshly built model inside the runner with the structurally pruned model.
    runner.model = model
